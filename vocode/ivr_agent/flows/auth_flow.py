...
async def ask_patient_provider(state: IVRState):
    ...
    chain = PROMPT['ROLE_CLASSIFIER_LOGIC_PROMPT'] | LLM.with_structured_output(RoleClassifierOutputSchema, include_raw=True)
    before_parsed = await chain.ainvoke({"user_input": user_input})
    parsed = before_parsed['parsed']
    ...
    new_retries = increment_retries(state, "patient_provider")
    if new_retries["retries"]["patient_provider"] > MAX_RETRIES: 
        return Command(goto="set_status_transfer")
    return Command(goto="ask_patient_provider", update=new_retries)

async def ask_npi(state: IVRState):
    retry_count = get_retries(state, "npi")
    msg = "Please say the NPI Number." if retry_count == 0 else "Please say the NPI again."
    
    user_input = interrupt({"message_to_play": msg, "input_type": "varied"})
    
    chain = PROMPT['NPI_EXTRACTOR_SCHEMA_PROMPT'] | LLM.with_structured_output(NPIExtractorOutputSchema, include_raw=True)
    before_parsed = await chain.ainvoke({"user_input": user_input})
    parsed = before_parsed['parsed']
    ...
    if not parsed.extracted_npi:
        new_retries = increment_retries(state, "npi")
        if new_retries["retries"]["npi"] > MAX_RETRIES: 
            return Command(goto="set_status_transfer")
...
auth_graph = asyncio.run(build_auth_subgraph())